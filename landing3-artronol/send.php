...
/* === 1) Конфигурация KMA === */
$KMA_API_URL = 'https://api.kma.biz/lead/add';
$KMA_API_KEY = 'fz-VV9YEAyiS5Lp_TqyWnwjb-5Ew-Xif';
$KMA_CHANNEL = 'I1iWdI'; // stream RO, offer 9831

/* --- ANTIDUP API --- */
$DEDUP_ENDPOINT = 'https://fixdoublecode.site/check-phone';
$DEDUP_PARTNER  = 'kma';
$DEDUP_OFFER    = 'Artronol';
$VERTICAL       = 'joints';
...
$full      = $_POST['full_phone'] ?? $_GET['full_phone'] ?? '';
...
$clickId   = $_POST['subid']      ?? $_GET['subid']      ?? '';
$utmSource = $_POST['utm_source'] ?? $_GET['utm_source'] ?? '';

/* === 2.1) Нормализация телефона в +E.164 === */
$DIAL_CODES = [
  // Europe
...
/* === 3) АНТИДУБЛЬ через fixdoublecode.site (с локальным фолбэком) === */
if ($phoneDigitsKey !== '') {
  $dedupPayloadArr = [
    'phone'   => $phoneE164,
    'geo'     => $country,
  ...
  $dedupCh = curl_init($DEDUP_ENDPOINT);
  curl_setopt_array($dedupCh, [
    CURLOPT_POST           => true,
  ...
  $dedupRespBody = curl_exec($dedupCh);
  $dedupHttpCode = curl_getinfo($dedupCh, CURLINFO_HTTP_CODE);
  $dedupErr      = curl_error($dedupCh);
  curl_close($dedupCh);

  $isDuplicate       = false;
  $usedFallback      = false;
  $hasValidApiAnswer = false;

  if ($dedupRespBody !== false && $dedupHttpCode >= 200 && $dedupHttpCode < 300) {
    $dedupData = json_decode($dedupRespBody, true);
    if (is_array($dedupData) && array_key_exists('isNew', $dedupData)) {
      $hasValidApiAnswer = true;
      $isDuplicate = !$dedupData['isNew'];
    }
  }

  if (!$hasValidApiAnswer) {
    @file_put_contents(
      __DIR__ . '/dedup.error.log',
      sprintf(
        "[%s] http=%s err=%s body=%s payload=%s\n",
        date('Y-m-d H:i:s'),
        (string)$dedupHttpCode,
        (string)$dedupErr,
        (string)$dedupRespBody,
  ...
  @file_put_contents(
    __DIR__ . '/dedup.log',
    sprintf(
      "[%s] dup=%s source=%s body=%s payload=%s\n",
      date('Y-m-d H:i:s'),
      $isDuplicate ? '1' : '0',
...
$hostUrl = $scheme . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost');

$acceptLanguage = $_SERVER['HTTP_ACCEPT_LANGUAGE'] ?? '';
$language = '';
if (preg_match('/^[a-z]{2}/i', $acceptLanguage, $languageMatch)) {
  $language = strtolower($languageMatch[0]);
}
...
$payload = [
  ...
  'is_mobile' => $isMobile ? 1 : null,
  'data1'     => $clickId ?: null,
];
$payload = array_filter($payload, static fn($value) => !($value === null || $value === ''));

@file_put_contents(
  __DIR__ . '/kma.request.log',
  sprintf("[%s] %s\n", date('Y-m-d H:i:s'), json_encode($payload, JSON_UNESCAPED_UNICODE)),
  FILE_APPEND
);

/* === 6) Немедленный редирект → фоновая обработка === */
...
@file_put_contents(
  __DIR__ . '/kma.response.log',
  sprintf("[%s] HTTP %s RESP: %s\n", date('Y-m-d H:i:s'), (string)$httpCode, ($responseBody ?: $errCurl)),
  FILE_APPEND
);

/* === 8) Facebook CAPI (фон, опционально) === */
